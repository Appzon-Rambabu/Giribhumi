...
function showArrays(event) {
    ...
    var url = "../api/ITDA/MastersLatlongslist";
    ...
    $.ajax(
     {
         ...
         contentType: "application/json; charset=utf-8",
            url: '../api/ITDA/MastersLatlongslist',
         data: "{'Ben_Id':'" + hidField + "'}",
         dataType: "json",
         success: function (response) {
             console.log(JSON.stringify(response));
             var Masterdist = response;
             for (var i = 0; i < Masterdist.itdalist.length; i++) {
                 var Itda = Masterdist.itdalist[0].a;
                 var district = Masterdist.itdalist[0].b;
                 var mandal = Masterdist.itdalist[0].c;
                 var village = Masterdist.itdalist[0].d;
                 var habitation = Masterdist.itdalist[0].e;
                 var benname = Masterdist.itdalist[0].f;
                 var extent = Masterdist.itdalist[0].g;
             }
             var contentString = '<b>Beneficiary Extent Land Details</b><br> <br>' +
             'Beneficary Name: ' + '<b>' + benname + '</b>' +
             '<br>' +
       'Exent: ' + '<b>' + extent + '</b>' +
    '<br>' +
    '<br>' +
...
function LatlongsLoad() {
 ...
 'Exent: ' + '<b>' + extent + '</b>' +
    ...
    '<br>' +
     ...
     'Habitation: ' + '<b>' + habitation + '</b>' +
     '<br>';
         document.getElementById('info1').innerHTML = contentString;
         },
         error: function (result) {
             alert("Error");
         }
