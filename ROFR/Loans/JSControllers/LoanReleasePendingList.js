(function () {
    ...
    app.controller("LoanReleasePendingList", ["$scope", "network_service", LoanRelPenList_CTRL]);

    function LoanRelPenList_CTRL(scope, ns) {
        var baseurl = "/API/LoansAPI/";

        scope.LOGIN_USER = sessionStorage.getItem("USER_ID");
        scope.pagename = "Loan Release Pending List";
        ...
        scope.GetDetails = function () {
            if (Validations()) {
                ...
                var req = { PTYPE: 4, FROM_DATE: moment(scope.FROM_DATE).format('YYYY/MM/DD'), TO_DATE: moment(scope.TO_DATE).format('YYYY/MM/DD'), LOGIN_USER: scope.LOGIN_USER }
