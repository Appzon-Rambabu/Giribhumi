(function () {
    ...
    app.controller("LoanApprovedList", ["$scope", "network_service", LoanAppList_CTRL]);

    function LoanAppList_CTRL(scope, ns) {
        var baseurl = "/API/LoansAPI/";

        scope.LOGIN_USER = sessionStorage.getItem("USER_ID");
        scope.pagename = "Loan Approved List";
        ...
        scope.GetDetails = function () {
            if (Validations()) {
                ...
                var req = { PTYPE: 1, FROM_DATE: moment(scope.FROM_DATE).format('YYYY/MM/DD'), TO_DATE: moment(scope.TO_DATE).format('YYYY/MM/DD'), LOGIN_USER: scope.LOGIN_USER }
