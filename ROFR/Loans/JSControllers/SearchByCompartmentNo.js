(function () {
    ...
    app.controller("SearchByCompartmentNo", ["$scope", "network_service", SearchCompart_CTRL]);

    function SearchCompart_CTRL(scope, ns) {
        var baseurl = "/API/LoansAPI/";

        scope.LOGIN_USER = sessionStorage.getItem("USER_ID");
        scope.pagename = "Loans Search By Compartment No";
        ...
        scope.BlockChange = function () {
            ...
            if (scope.SelDistrict && scope.SelDivision && scope.SelRange && scope.SelBeet && scope.SelBlock)
                LoadCommonData("10"); // Load Compartment Data
        }
        ...
        scope.GetDetails = function () {
            ...
            if (GetDetValidations()) {
                scope.PattadarTable = [];
                LoadCommonData("11"); // Load Former Data
            }
        }
        ...
        scope.GetPattadarDetails = function (landid) {
            if (landid) {
                ...
                if (scope.CurrBen.length > 0) {
                    LoadCommonData("12", landid); // Fill the Pattadar Data
                }
            }
        }
        ...
        function LoadCommonData(type, landid) {
            ...
            if (ftype == "5")
                ...
                req = { PTYPE: 10, DISTRICT: scope.SelDistrict, FOREST_DIVISION: scope.SelDivision, FOREST_RANGE: scope.SelRange, FOREST_BEAT: scope.SelBeet, FOREST_BLOCK: scope.SelBlock }
            else if (ftype == "11")
                req = { PTYPE: 11, DISTRICT: scope.SelDistrict, FOREST_DIVISION: scope.SelDivision, FOREST_RANGE: scope.SelRange, FOREST_BEAT: scope.SelBeet, FOREST_BLOCK: scope.SelBlock, COMPARTMENT_NO: scope.SelCompartment, LOGIN_USER: scope.LOGIN_USER }
            else if (ftype == "12")
                req = { PTYPE: 12, LAND_ID: landid}
            else if (ftype == "16")
