(function () {
    ...
    app.controller("LoanReleaseEntry", ["$scope", "network_service", LoanRelEntry_CTRL]);

    function LoanRelEntry_CTRL(scope, ns) {
        var baseurl = "/API/LoansAPI/";

        scope.LOGIN_USER = sessionStorage.getItem("USER_ID");
        scope.pagename = "Loan Release Entry";
        ...
        scope.GetDetails = function () {
            if (GetDetValidations()) {
                ...
                if (scope.ROFR_PATTADAAR_ACCOUNT_NO)
                    LoadCommonData("16"); // Load Former Data
            }
        }
        ...
        scope.GetPattadarDetails = function (landid) {
            if (landid) {
                ...
                LoadCommonData("17", landid); // Fill the Pattadar Details
            }
        }
        ...
        scope.SaveData = function () {
            if (MainValidations()) {
                ...
                var req = { PTYPE: 18, LAND_ID: scope.CurrBen[0].LAND_ID, CHARGE_RELEASE_REMARKS: scope.CHARGE_RELEASE_REMARKS, LOGIN_USER: scope.LOGIN_USER }
                ns.post(baseurl + "SaveCreationData", req, function (value) {
                    scope.preloader = false;
                    if (value.data.Status == 100) {
                        swal("info", "Loan Release Entry Saved Sucessfully", "info");
        ...
        function LoadAllData() {
            LoadCommonData("24"); // Load Accounts Data
        ...
        function LoadCommonData(type, landid) {
            ...
            if (ftype == "11")
                ...
                req = { PTYPE: 16, LOAN_ACCOUNTNO: scope.ROFR_PATTADAAR_ACCOUNT_NO }
            else if (ftype == "24")
                req = { PTYPE: 24, LOGIN_USER: scope.LOGIN_USER }
            else if (ftype == "17")
                req = { PTYPE: 17, LAND_ID: landid }
                ...
                if (value.data.Status == 100) {
                    ...
                    if (ftype == "11") {
                        ...
                        if (value.data.Data.length > 0) {
                            ...
                            scope.CHARGE_CREATION_APP_ENTRYBY = currdata.CHARGE_CREATION_APP_ENTRYBY;
                            scope.CHARGE_CREATION_APP_DATE = currdata.CHARGE_CREATION_APP_DATE;
