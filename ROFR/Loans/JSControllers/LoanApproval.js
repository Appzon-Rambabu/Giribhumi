(function () {
    ...
    app.controller("LoanApproval", ["$scope", "network_service", LoanAppr_CTRL]);

    function LoanAppr_CTRL(scope, ns) {
        var baseurl = "/API/LoansAPI/";

        scope.LOGIN_USER = sessionStorage.getItem("USER_ID");
        scope.pagename = "Loan Information Approval";
        ...
        scope.GetDetails = function () {
            if (GetDetValidations()) {
                ...
                if (scope.ROFR_PATTADAAR_ACCOUNT_NO) {
                    LoadCommonData("13"); // Load Former Data
                }
                else if (scope.SEL_FROM_DATE && scope.SEL_TO_DATE) {
                    LoadCommonData("23"); // Load Former Data
                }
            }
        }
        ...
        scope.GetPattadarDetails = function (landid) {
            ...
            if (landid) {
                ...
                LoadCommonData("14", landid); // Fill the Pattadar Details
                LoadCommonData("27", landid); // Load Ongoing Loans Data
                LoadCommonData("28", landid); // Load History Loans Data
            }
        }
        ...
        scope.ChangeHistory = function () {
            ...
            //    if (scope.CurrType == "1")
            //        LoadCommonData("9", scope.CurrBen[0].LAND_ID); // Load Ongoing Loans Data
            //    else
            //        LoadCommonData("10", scope.CurrBen[0].LAND_ID); // Load History Loans Data
            //}
        }
        ...
        scope.SaveData = function (type) {
            if (MainValidations()) {
                ...
                var req = { PTYPE: 15, LAND_ID: scope.CurrBen[0].LAND_ID, CHARGE_CREATION_APP_REJ_REMARKS: scope.CHARGE_CREATION_APP_REJ_REMARKS, CHARGE_CREATION_APP_STATUS: type, LOGIN_USER: scope.LOGIN_USER }
                ns.post(baseurl + "SaveCreationData", req, function (value) {
                    scope.preloader = false;
                    if (value.data.Status == 100) {
                        swal("info", "Loan Information Status Updated Sucessfully", "info");
        ...
        function LoadAllData() {
            LoadCommonData("22"); // Load Accounts Data
        ...
        function LoadCommonData(type, landid) {
            ...
            if (ftype == "11")
                ...
                req = { PTYPE: 13, LOAN_ACCOUNTNO: scope.ROFR_PATTADAAR_ACCOUNT_NO }
            else if (ftype == "22")
                req = { PTYPE: 22, LOGIN_USER: scope.LOGIN_USER }
            else if (ftype == "23")
                req = { PTYPE: 23, FROM_DATE: moment(scope.SEL_FROM_DATE).format('YYYY/MM/DD'), TO_DATE: moment(scope.SEL_TO_DATE).format('YYYY/MM/DD'), LOGIN_USER: scope.LOGIN_USER }
            else if (ftype == "14")
                req = { PTYPE: 14, LAND_ID: landid }
            else if (ftype == "27")
                req = { PTYPE: 27, LAND_ID: landid, LOGIN_USER: scope.LOGIN_USER }
            else if (ftype == "28")
                req = { PTYPE: 28, LAND_ID: landid, LOGIN_USER: scope.LOGIN_USER }
            ////else if (ftype == "27")
            ////    req = { PTYPE: 27, LOAN_ACCOUNTNO: scope.ROFR_PATTADAAR_ACCOUNT_NO }
            //else if (ftype == "28")
            //    req = { PTYPE: 28, FROM_DATE: moment(scope.SEL_FROM_DATE).format('YYYY/MM/DD'), TO_DATE: moment(scope.SEL_TO_DATE).format('YYYY/MM/DD'), LOGIN_USER: scope.LOGIN_USER }
                ...
                if (value.data.Status == 100) {
                    ...
                    if (ftype == "11") {
                        ...
                        if (value.data.Data.length > 0) {
                            ...
                            scope.CHARGE_CREATED_BY = currdata.CHARGE_CREATED_BY;
                            if (currdata.CHARGE_CREATED_ON)
                                scope.CHARGE_CREATED_ON = new Date(currdata.CHARGE_CREATED_ON);
                        }
                        else
        ...
        function MainValidations() {
            if (!scope.CurrBen.length) {
                ...
                swal("Info", "Please Enter Approval/Reject Remarks ", "info");
                return false;
            }
            return true;
        }
